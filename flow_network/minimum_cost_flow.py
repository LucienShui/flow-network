...
class MinimumCostFlow(Clib):
    def __init__(self, n: int):
        ...
        self._clib.minimum_cost_flow_new.restype = ctypes.POINTER(ctypes.c_void_p)
        self.__obj = ...
    ...
    def add_edge(self, u: int, v: int, flow: int, cost: int) -> None:
        """
        add edge from u to v with flow and cost into python object's cache
    ...
    def __add_edge(self, u: int, v: int, flow: int, cost: int) -> None:
        """
        add edge from u to v with flow and cost into C++ object
        :param u: point's index
        :param v: point's index
        :param flow: edge capacity
        :param cost: cost for cutting an edge
    ...
    def __flush(self):
        c_n: ctypes.c_int = c_type_transfer(self.__n)
        c_m: ctypes.c_int = c_type_transfer(len(self.edges))

        self.__obj = self._clib.minimum_cost_flow_new(c_n, c_m)

        for i in range(0, len(self.edges), 2):
            self.__add_edge(*self.edges[i])
